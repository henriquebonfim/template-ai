...
swaggerui_blueprint = get_swaggerui_blueprint(
    SWAGGER_URL, API_URL, config={"app_name": "Template-AI API"}
)
...
def validate_environment():
    ...
    llm_base_url = os.getenv("AI_MODEL_URL", "")
    llm_model_name = os.getenv("LLM_MODEL_NAME", "")

    if not llm_base_url:
        app.logger.warning("AI_MODEL_URL is not set. API calls will fail.")
...
def call_llm_api(user_message):
    ...
    response = requests.post(
        get_llm_endpoint(), headers=headers, json=chat_request, timeout=30
    )
...
def add_security_headers(response):
    ...
    response.headers["Content-Security-Policy"] = (
        "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; font-src 'self' https://cdnjs.cloudflare.com"
    )
    return response
...
if __name__ == "__main__":
    ...
    if not env_valid:
        app.logger.warning(
            "Environment not fully configured. Some features may not work."
        )
